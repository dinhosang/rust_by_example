...
//      Once the collection has been consumed it is no longer available for
//          reuse as it has been 'moved' within the loop.
fn for_and_interators_into_iter() {
    let names = vec!["Bob", "Frank", "Ferris"];
...
fn for_and_interators_iter_mut() {
...
pub fn for_loop_examples() {
    for_and_range();
    for_and_interators_iter();
    for_and_interators_into_iter();
    for_and_interators_iter_mut();
}
